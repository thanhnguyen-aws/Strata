...
-/
def BufferM (α : Type) := ReaderT ByteArray (Except (Nat × Error)) α

namespace BufferM

instance : Inhabited (BufferM α) where
  default := fun _ => .error (0, "")

instance : Monad BufferM := by unfold BufferM; exact inferInstance
...
instance (x y : Fuel) : Decidable (x ≤ y) := inferInstanceAs (Decidable (x.toNat ≤ y.toNat))
...
instance : OfNat Fuel x where
  ofNat := x

instance : HSub Fuel Nat Fuel where
  hSub x y := x.toNat - y

def sub_le (f : Fuel) (n : Nat) : f - n ≤ f := Nat.sub_le f n

instance : HSub Nat Fuel Nat where
  hSub x y := x - y.toNat
...
/-- This shows that we -/
theorem meet_trans {m n : Progress} : (a, b) ⊧ m → (b, c) ⊧ n → (a, c) ⊧ meet m n := by
...
def Pre (f : Fuel) (m : Progress) : Type := { x : Fuel // (f, x) ⊧ m }
...
def Reader (m : Progress) α := ∀(f : Fuel), BufferM (α × f.Pre m)
...
protected def BufferM.ofReader (fuel : Fuel) (act : Reader m α) : BufferM (α × fuel.Pre m) :=
  act fuel

namespace Reader

protected def pure (a : α) : Reader .any α := fun f => pure (a, f.unchanged)

protected def map (f : α → β) (h : Reader m α) : Reader m β := fun fuel =>
  (fun (a, f1) => (f a, f1)) <$> h fuel

protected def bind (g : Reader m α) (h : α → Reader n β) : Reader (.meet m n) β := fun f => do
  let (a, ⟨f1, f1p⟩) ← g f
  let (b, ⟨f2, f2p⟩) ← h a f1
...
-/
protected def bindAny (g : SReader α) (h : α → AReader β) : SReader β := .bind g h
...
protected def fail (off : Nat) (msg : String) : Reader m α := fun _ => .fail off msg
...
def readUpto.aux (init : Fuel) (act : ∀(fuel : init.Pre .any), α → BufferM (α × fuel.val.Pre .strict)) (v : α) (fuel : init.Pre .any) (limit : Nat) : BufferM (α × init.Pre .any) := do
...
def readUpto (fuel : Fuel) (init : α) (limit : Nat) (act : ∀(fuel : fuel.Pre .any), α → BufferM (α × fuel.val.Pre .strict)) : BufferM (α × fuel.Pre .any) := do
  readUpto.aux fuel act init fuel.unchanged limit

def readSeqUpto (fuel : Fuel) (limit : Nat) (act : ∀(fuel : fuel.Pre .any), BufferM (α × fuel.val.Pre .strict)) : BufferM (Array α × fuel.Pre .any) := do
  readUpto fuel #[] limit fun fuel a => (fun (v, p) => (a.push v, p)) <$> act fuel

def readWhile (fuel : Fuel) (init : α) (act : α → UInt8 → Step α β) : BufferM (β × fuel.Pre .strict) := aux fuel.unchanged init
...
def readUpto (init : α) (limit : Nat) (act : α → SReader α) : AReader α :=
  .ofM fun fuel => .readUpto fuel init limit (fun fuel a => .ofReader fuel.val (act a))

def readSeqUpto (limit : Nat) (act : SReader α) : AReader (Array α) :=
  .ofM fun fuel => .readSeqUpto fuel limit (.ofReader ·.val act)

def readWhile (init : α) (act : α → UInt8 → Step α β) : SReader β :=
