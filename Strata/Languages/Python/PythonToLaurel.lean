  ...
  | .Attribute _ obj attr _ => do
    ...
    | .Name _ name _ =>
      if name.val == "self" && ctx.currentClassName.isSome then
        ...
        --  return unboxExpr
        if ty == "Any" || isCompositeType ctx ty then
          let readFieldExpr := mkStmtExprMd $ .FieldSelect objExprCom attr.val AnyTy
    ...
    | .Attribute _ obj attr _ =>
      ...
      | .Name _ name _ =>
        if name.val == "self" && ctx.currentClassName.isSome then
          ...
          --let objExprCom := mkStmtExprMd (.StaticCall "Any..as_composite!" [objExpr])
          let ty ← inferExprType ctx obj
          let assignStmt :=  if ty == "Any" || isCompositeType ctx ty then
            mkStmtExprMdWithLoc (StmtExpr.Assign [targetExpr] rhs_trans) md
          else
            mkStmtExprMdWithLoc (StmtExpr.Assign [targetExpr] rhs_trans) md
          return (ctx, [assignStmt], true)
...
partial def getExceptionAssertions (ctx : TranslationContext) (e : StmtExprMd) : List StmtExprMd :=
  ...
  maybeExceptExprs.map fun mbe => match mbe.val with
  | .FieldSelect target attr _ =>
    let field := mkStmtExprMd $ .StaticCall "LaurelFieldConstruct" [
      target,
      mkStmtExprMd $ .LiteralString attr.text
    ]
    ...
    mkStmtExprMdWithLoc (.Assert fieldIsNotInvalid) mbe.md
  | _ =>
